### 1. Сначала вам нужно клонировать этот репозиторий в свой локальный компьютер с помощью команды:
> git clone https://github.com/BasharBetta7/python_programming_task_11.git
### 2. Перейдите по указанному вами пути на вашем локальном компьютере и убедитесь, что вы находитесь в каталоге "./python_programming_task_11":
> cd ./python_programming_task_11
### 3. Создайте виртуальную среду с помощью: 
> python -m venv venv
### 4. Активируйте созданную среду:
> source venv/bin/activate
### 5. Установить зависимости:
> pip install --upgrade pip && pip install -r requirements.txt

если вы столкнулись с какими-либо ошибками при выполнении предыдущей команды, вы можете вручную установить зависимости, выполнив:
 > pip install --upgrade pip && pip install pytest pylint mypy pytest-mock


Теперь вы можете приступить к решению задач. При запуске скриптов убедитесь, что вы находитесь внутри каталога "./python_programming_task_11", в противном случае у вас могут возникнуть проблемы с импортом модулей.

После решения каждой задачи в соответствии с описанием, приведенным в LMS, вы должны запустить скрипт проверки (информация приведена в домашнем задании LMS).

Удачи!